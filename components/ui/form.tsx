...
const useFormField = () => {
  ...
  try {
    ...
    if (!itemContext) {
      console.warn("useFormField should be used within <FormItem>")
      return {
        id: "fallback-id",
...
>(({ className, ...props }, ref) => {
  ...
  const formItemId = field.formItemId
...
>(({ ...props }, ref) => {
  const field = useFormField()
  const error = 'error' in field ? field.error : undefined
  const { formItemId, formDescriptionId, formMessageId } = field
